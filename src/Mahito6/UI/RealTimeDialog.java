...
//		g.drawImage(img.getSubimage(Math.max(0, nowx-range), Math.max(0, nowy-range), range*2, range*2), 0, 0, earth);
		int cutx = nowx + coord.minx - Constants.imagePositionOffset/2;
		int cuty = nowy + coord.miny - Constants.imagePositionOffset/2;
		g.drawImage(owner.getProblem().getSubImage(cutx, cuty, range), 0, 0, earth);
		g.dispose();
